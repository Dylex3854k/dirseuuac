...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $cargo = $con->real_escape_string(htmlentities($_POST['cargo']));
    ...
    // Solo insertar en personalDir si el nivel es "personal" o "coordinador"
    if ($nivel === 'PERSONAL' || $nivel === 'COORDINADOR') {
        $ins2 = $con->query("INSERT INTO personaldir (nombreCom, cargo) VALUES ('$nombre', '$cargo')");
        if ($ins2) {
            $perDIR_id = $con->insert_id;
            $pass1 = sha1($pass1);

            $ins = $con->query("INSERT INTO usuario (nick, nombre, pass, correo, nivel, perDIR_id) VALUES ('$nick', '$nombre', '$pass1', '$correo', '$nivel', '$perDIR_id')");
            if ($ins) {
            ...
            header('location:../extend/alerta.php?msj=Error al registrar en personaldir&c=us&p=in&t=error');
        }
    } else {
        $pass1 = sha1($pass1);

        $ins = $con->query("INSERT INTO usuario (nick, nombre, pass, correo, nivel) VALUES ('$nick', '$nombre', '$pass1', '$correo', '$nivel')");
        if ($ins) {
            header('location:../extend/alerta.php?msj=El usuario ha sido registrado&c=us&p=in&t=success');
        } else {
            header('location:../extend/alerta.php?msj=El usuario no pudo ser registrado&c=us&p=in&t=error');
